    ...
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/towns" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="size-4" /> Back to local
        </Link>
        <form action="/search" method="get" className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-1.5">
          ...
          <input name="q" defaultValue={query} placeholder="Search local first…" className="w-44 bg-transparent text-sm focus:outline-none sm:w-56" />
        </form>
      </div>

      <header className="mt-6">
        <h1 className="font-serif text-3xl font-semibold">Can&apos;t find it locally?</h1>
        <p className="mt-2 max-w-2xl text-muted-foreground">
          When a local shop doesn&apos;t carry it yet, you can grab it on our Amazon store — and the
          proceeds come back to support this platform and the small businesses on it.
        </p>
      </header>
      ...
      {query && (
        <div className="mt-6 rounded-xl border border-border bg-card p-5">
          ...
          >
            See “{query}” on Amazon <ExternalLink className="size-4" />
          </AmazonLink>
        </div>
      )}

      {/* Category tiles */}
      <h2 className="mt-8 font-serif text-lg font-semibold">Browse categories</h2>
      ...
      {storefront && (
        <div className="mt-8">
          ...
          >
            Visit our full Amazon storefront <ExternalLink className="size-4" />
          </AmazonLink>
        </div>
      )}
      ...
      <p className="mt-10 text-xs text-muted-foreground">
        As an Amazon Associate, MainStreet earns from qualifying purchases. Links open on
        amazon.com in a new tab — your MainStreet tab stays right here.
      </p>
    </div>
  );
}
