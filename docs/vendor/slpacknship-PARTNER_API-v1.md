...
| `402` | payment not verified | the PaymentIntent isn't succeeded, doesn't cover the retail, or doesn't match this quote — confirm the charge before retrying |
| `409` | quote expired or already used | call `/rates` again for a fresh quote |
| `422` | validation error `{ "error", "field" }` | fix the field |
| `429` | rate-limited | honor `Retry-After` |
| `500`/`502` | server or carrier error | safe to retry the SAME request; a label is only produced once per quote |

**Idempotency / retries:** a quote is single-use, so retrying `/shipments` after
a `5xx` will either succeed once or return `409` if the first attempt already
produced the label — you will never be double-charged a carrier label for one
quote. If you need strict idempotency, retry with the same `quoteId`.
